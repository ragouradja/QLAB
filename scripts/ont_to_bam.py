...
def get_args():
    ...
    parser.add_argument("--file","-f", help="Path to file with reads informations for one chromosom.", type=str, required=True, metavar="chr_file")
...
def get_chrs_path(args, sam_output, bam_basename):
    ...
    print("Processing all chromosoms in parallel.")
    for i in range(1,6):
...
def get_dict(read_file, chrom_name):
    ...
    start_pl = time.time()
    meth_dtf = pl.read_csv(read_file, new_columns  = header, sep = "\t", has_header = False,low_memory = True ).drop("start_in_read").to_pandas()
    ...
    print(f"{chrom_name} :: Dataframe loaded in {get_time(start_time)}s")
    start_time = time.time()
...
def main_fast(bedfile_path, SAM_output, BAM_output, genome, args):
    begin_chr = time.time()
...
def main_slow(bedfile_path, SAM_output, BAM_output, genome, args):
    begin_chr = time.time()
    chrom_name = re.search("chr\d", bedfile_path).group(0).capitalize()
...
if __name__ == "__main__":
    ...
    if args.all:
        ...
        (bedfile_path, SAM_path, BAM_path, genome, args) for bedfile_path, SAM_path, BAM_path in list_files)
    else:
        run_function(bedfile, SAM_output, BAM_output, genome, args)
